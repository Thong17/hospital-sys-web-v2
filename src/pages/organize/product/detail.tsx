...
import { getProductDelete, getProductDetail } from 'stores/product/action'
...
const ProductDetail = () => {
  ...
  const handleEdit = (
    ...
    navigate(`/organize/product/update/${id}`)
  }

  const handleClickHistory = () => {
    navigate(`/organize/product/detail/${id}/history`)
  }
  ...
  const handleDelete = (
    ...
    confirm({
      ...
      .then((confirmData: any) => {
        dispatch(getProductDelete({ id: id, reason: confirmData?.reason }))
          .unwrap()
          .then(() => navigate('/organize/product'))
          .catch(console.error)
      })
      .catch(() => {})
  }
  ...
  return (
      ...
      navbar={
        <Breadcrumb
          list={[
            ...
            {
              id: 'action',
              href: `/organize/product/detail/${id}`,
    ...
    >
      <Container>
        <TitleContainer text={translate('TITLE_PRODUCT_DETAIL') as String}>
        ...
        <Stack direction={'column'} mb={2} sx={{ paddingTop: '20px', '& .section-container': { marginTop: '20px' } }}>
          <Stack sx={{ width: '100%' }} direction={'row'}>
            <LabelDetail label={translate('PRODUCTNAME') as String}>
